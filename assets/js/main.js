(function($) {
  ...
  $(".aside-trigger-right").on('click', function() {
    ...
    $el.toggleClass('open');
    // CSS-based fade (replaces jQuery .fadeIn/.fadeOut) — .cssfx-fade + .is-open handle the animation
    var $sidebar = $el.find('.sidebar');
    $sidebar.addClass('cssfx-fade');
    if ($el.hasClass('open')) {
      setTimeout(function() { $sidebar.addClass('is-open'); }, 300);
    } else {
  ...
  $(".sigma_notice a").on('click', function(e) {
    e.preventDefault();
    // CSS-based slide (replaces jQuery .slideToggle)
    $(this).closest('.sigma_notice').next().addClass('cssfx-slide').toggleClass('is-open');
  });
  ...
  $(".sigma_progress").each(function() {
    ...
    // CSS-based animation (replaces jQuery .animate) — width + left use CSS transitions
    $(progressBar).one('inview', function(event, isInView) {
      if (isInView) {
        var pct = $(progressBar).attr("aria-valuenow") + "%";
        progressBar.css({'transition': 'width .8s ease', 'width': pct});
        setTimeout(function() {
          progressCount.css({'transition': 'left .6s ease, opacity .6s ease', 'left': pct, 'opacity': 1});
        }, 800);
      }
    });
  });
  ...
  $('body').on('click', '.sigma_top', function() {
    // Native smooth scroll (replaces jQuery .animate) — no render jank
  ...
  // Collapse Theme Color Panel + Store Widget by default on mobile (<768px)
  // Runs on DOM ready because the widgets live AFTER this script tag in HTML.
  // User can still open them by tapping the toggle tabs.
  $(function () {
    if (window.matchMedia('(max-width: 767.98px)').matches) {
      $('.mht-color-theme').addClass('mht-slide-out');
      $('.mht-buy-widget').removeClass('mht-buy-open');
    }
  });
